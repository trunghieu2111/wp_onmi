<?php
/**
 * Vandana Lite Customizer Repeater Control.
 * 
 * @package Vandana Lite
*/
...
if( ! class_exists( 'Vandana_Lite_Control_Repeater' ) ) {
    /**
     * Repeater control
     */
    class Vandana_Lite_Control_Repeater extends WP_Customize_Control {
    	 ...
    	 * @var string
    	 */
    	public $type = 'vandana_lite_repeater';
    	...
    	public function __construct( $manager, $id, $args = array() ) {
    		...
    		$this->row_label = array(
    			'type' => 'text',
    			'value' => esc_attr__( 'row', 'vandana-lite' ),
    			'field' => false,
    		);
    		...
    		if ( empty( $this->button_label ) ) {
    			$this->button_label = sprintf( esc_attr__( 'Add new %s', 'vandana-lite' ), $this->row_label['value'] );
    		}
    	...
    	public function enqueue() {    
            ...
            wp_enqueue_script( 'sarada-repeater', get_template_directory_uri() . '/inc/custom-controls/repeater/repeater.js', array( 'jquery', 'jquery-ui-core', 'jquery-ui-sortable' ), false, true );
    		wp_enqueue_style( 'sarada-repeater', get_template_directory_uri() . '/inc/custom-controls/repeater/repeater.css', null );
    	}
    	...
    	protected function render_content() {
    		...
    		<?php if ( isset( $this->choices['limit'] ) ) : ?>
    			<p class="limit"><?php printf( esc_html__( 'Limit: %s rows', 'vandana-lite' ), esc_html( $this->choices['limit'] ) ); ?></p>
    		<?php endif; ?>
    		...
    		<script type="text/html" class="customize-control-repeater-content">
    			...
    			<li class="repeater-row minimized" data-row="{{{ index }}}">
    				...
    				<div class="repeater-row-content">
    					<# _.each( data, function( field, i ) { #>
    						...
    						<div class="repeater-field repeater-field-{{{ field.type }}}">
    							...
    							<# } else if ( 'color' === field.type ) { #>
    						        ...
    						        <label>
    						            ...
    						            <# if ( field.description ) { #>
    						                <span class="description customize-control-description">{{{ field.description }}}</span>
    						            <# } #>
    						            <input class="color-picker-hex" type="text" maxlength="7" placeholder="<?php echo esc_attr__( 'Hex Value', 'vandana-lite' ); ?>"  value="{{{ field.default }}}" data-field="{{{ field.id }}}" {{ defaultValue }} />
    							...
    							<# } else if ( field.type === 'image' || field.type === 'cropped_image' ) { #>
    								...
    								<figure class="sarada-image-attachment" data-placeholder="<?php esc_attr_e( 'No Image Selected', 'vandana-lite' ); ?>" >
    									<# if ( field.default ) { #>
    										<# var defaultImageURL = ( field.default.url ) ? field.default.url : field.default; #>
    										<img src="{{{ defaultImageURL }}}">
    									<# } else { #>
    										<?php esc_html_e( 'No Image Selected', 'vandana-lite' ); ?>
    									<# } #>
    								</figure>
    
    								<div class="actions">
    									<button type="button" class="button remove-button<# if ( ! field.default ) { #> hidden<# } #>"><?php esc_html_e( 'Remove', 'vandana-lite' ); ?></button>
    									<button type="button" class="button upload-button" data-label=" <?php esc_attr_e( 'Add Image', 'vandana-lite' ); ?>" data-alt-label="<?php esc_attr_e( 'Change Image', 'vandana-lite' ); ?>" >
    										<# if ( field.default ) { #>
    											<?php esc_html_e( 'Change Image', 'vandana-lite' ); ?>
    										<# } else { #>
    											<?php esc_html_e( 'Add Image', 'vandana-lite' ); ?>
    										<# } #>
    									</button>
    									<# if ( field.default.id ) { #>
    							...
    							<# } else if ( field.type === 'upload' ) { #>
    								...
    								<figure class="sarada-file-attachment" data-placeholder="<?php esc_attr_e( 'No File Selected', 'vandana-lite' ); ?>" >
    									<# if ( field.default ) { #>
    									...
    									<# } else { #>
    										<?php esc_html_e( 'No File Selected', 'vandana-lite' ); ?>
    									<# } #>
    								</figure>
    
    								<div class="actions">
    									<button type="button" class="button remove-button<# if ( ! field.default ) { #> hidden<# } #>"></button>
    									<button type="button" class="button upload-button" data-label="<?php esc_attr_e( 'Add File', 'vandana-lite' ); ?>" data-alt-label="<?php esc_attr_e( 'Change File', 'vandana-lite' ); ?>" >
    										<# if ( field.default ) { #>
    											<?php esc_html_e( 'Change File', 'vandana-lite' ); ?>
    										<# } else { #>
    											<?php esc_html_e( 'Add File', 'vandana-lite' ); ?>
    										<# } #>
    									</button>
    									<# if ( field.default.id ) { #>
    										<input type="hidden" class="hidden-field" value="{{{ field.default.id }}}" data-field="{{{ field.id }}}" >
    					...
    					<# }); #>
    					<button type="button" class="button-link repeater-row-remove"><?php esc_html_e( 'Remove', 'vandana-lite' ); ?></button>
    				</div>
    			</li>
    		</script>
    		<?php
