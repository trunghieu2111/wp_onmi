...
wp_enqueue_script( 'thememove-js-maps', sprintf( 'http://maps.google.com/maps/api/js?key=%s&amp;language=en', $google_api ) );
wp_enqueue_script( 'thememove-js-gmap3', TM_HELI_THEME_ROOT . '/js/gmap3.min.js' );
...
if ( $el_class ) {
	echo 'class="thememove-gmaps ' . esc_attr( $el_class ). '"';
} else {
			...
			if (gmMapDiv.length) {
				...
				gmMapDiv.gmap3({
					...
					marker: {
						...
						options: {
							<?php if ($marker_icon == '') { ?>
							icon: "<?php echo TM_HELI_THEME_ROOT ?>/assets/images/map-marker.png",
							<?php } else { ?>
						...
						events: {
							click: function (marker, event) {
								...
								if (infowindow) {
									...
									infowindow.setContent("<?php echo esc_js( $content ); ?>");
								}
								else {
									$(this).gmap3({
										infowindow: {
											anchor: marker,
											options: {content: "<?php echo esc_js( $content ); ?>"}
										}
									});
								}
							}
