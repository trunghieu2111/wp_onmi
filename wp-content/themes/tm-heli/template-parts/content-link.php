...
<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
		...
		<div class="entry-meta">
			<div class="container row row-xs-center">
				<div class="info col-sm-9 col-md-10">
					...
					<span class="categories-links"><i class="fa fa-folder"></i> <?php echo esc_html ( __( 'In ', 'tm-heli' ) . get_the_category_list( ',') )?> </span>
	...
	<header class="entry-header">
		<?php
		$custom_link = get_post_meta( get_the_ID(), 'infinity_post_custom_link', true );
		if ( $custom_link ) {
			the_title( sprintf( '<h2 class="entry-title"><a href="%s" rel="bookmark">', esc_url( $custom_link ) ), '</a></h2>' );
		} else {
