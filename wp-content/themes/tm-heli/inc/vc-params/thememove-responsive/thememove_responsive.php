...
class ThemeMove_Responsive {
	...
	public function __construct( $settings, $value ) {
		...
		$this->item_width_list = array(
			__( '1 columns - 1/1 - 12 items in a row', 'tm-heli' ) => 1,
			__( '2 columns - 1/6 - 6 items in a row', 'tm-heli' )  => 2,
			__( '3 columns - 1/4 - 4 items in a row', 'tm-heli' )  => 3,
			__( '4 columns - 1/3 - 3 items in a row', 'tm-heli' )  => 4,
			__( '6 columns - 1/2 - 2 items in a row', 'tm-heli' )  => 6,
			__( '12 columns - 1/1 - 1 item in a row', 'tm-heli' )  => 12,
		);
	}
	...
	private function sizeControl( $size_key ) {
		if ( 'md' === $size_key ) {
			return '<span class="vc_description">' . __( 'Default value from "Number of items in a row" attribute', 'tm-heli' ) . '</span>';
		}

		$empty_label = $size_key === 'xs' ? '' : __( 'Inherit from smaller', 'tm-heli' );
		$output      = '<select name="tm_col_' . $size_key . '_size" class="thememove_responsive_field" data-type="size-' . $size_key . '">' . '<option value="" style="color: #ccc;">' . $empty_label . '</option>';
	...
	public function render() {
		...
		$html .= '<div class="tm_responsive" data-responsive="true">
             ...
             <table class="vc_table tm_table tm_responsive-table">
              <tr>
                <th>' . __( 'Device', 'tm-heli' ) . '</th>
                <th>' . __( 'Item width', 'tm-heli' ) . '</th>
                <th>' . __( 'Hide on device?', 'tm-heli' ) . '</th>
              </tr>
            ';
          ...
          <td>
					<label>
						<input type="checkbox" name="vc_hidden-' . $key . '"
						       value="yes"' . ( in_array( 'vc_hidden-' . $key, $this->valueData() ) ? ' checked="true"' : "" ) . 'class="thememove_responsive_field">
					</label>
				</td>
        </tr>
      ';
...
//vc_add_shortcode_param( 'thememove_responsive', 'thememove_responsive_settings_field', TM_HELI_THEME_ROOT . '/inc/vc-params/thememove-responsive/thememove_responsive.js' ); todo
